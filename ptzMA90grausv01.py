# Satellite Tracker - Code malha aberta - estudar porque esta hora aumenta, hora diminui az e elev
'''
Autores: 
Adalto Myiai
Guilherme Marra
...
class tracking:
  ...
  def getSatData(self):
    ...
    self.spaz = 40 #Para MA
    self.spel = 40 #Para MA
  ...
  def azelTRK(self):
    ...
    if self.spel <= 90 and self.spel > 0 and self.spaz >= 0 and self.spaz < 332:	#Deixando 332 como limite de azimute
      ...
      if self.azheading < 0 and self.spaz <= 180:
        #self.ptz.write(self.left)	#Sentido Antihorario Azimute Down
        #time.sleep((180-self.spaz)*self.AZ)	#Partindo de 180 graus3
        pass
      #azhalfup
      if self.azheading >= 0 and self.spaz > 180:
        #self.ptz.write(self.left)	#Sentido Antihorario Azimute Down
...
while 1:
  ...
  tr1.getSatData()
  tr1.sentidoAz()
  if True: #tr1.chosensat.range <= 3000000:
    tr1.startAll()
    tr1.azelTRK()
    time.sleep(2)
    #tr1.ptz.close
